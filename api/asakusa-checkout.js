// POST /api/asakusa-checkout
// Creates a Stripe Checkout session for MKG HOTEL 浅草 (hotel 86177544)
...
async function getVerifiedPrice(checkIn, checkOut, adultCount, roomTypeId) {
  try {
    ...
    // Query all rooms (same as client), then filter by roomId.
...
module.exports = async function handler(req, res) {
  ...
  const productName = roomName
    ? `MKG HOTEL 浅草 · ${roomName} · ${nights}泊`
    : `MKG HOTEL 浅草 · ${nights}泊`;
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${BASE}/hotel-asakusa.html?booking=success&sid={CHECKOUT_SESSION_ID}`,
      cancel_url:  `${BASE}/hotel-asakusa.html?booking=cancelled`,
      locale: 'ja',
      metadata: {
        checkIn,
        checkOut,
